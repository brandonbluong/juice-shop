...
export class ScoreBoardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.configurationService.getApplicationConfiguration().subscribe(
      (config) => {
        ...
        this.codingChallengesEnabled =
          config.challenges.codingChallengesEnabled;
  ...
  trustDescriptionHtml(challenge: Challenge) {
    challenge.description = this.sanitizer.bypassSecurityTrustHtml(
      escapeHtml(challenge.description as string)
    );
  }
  ...
  calculateCodingProgressPercentage() {
    ...
    this.percentCodingChallengesSolved = (
      (100 * codingChallengeProgress) /
      (numCodingChallenges * 2)
    ).toFixed(0);
  }
      ...
      for (const challenge of this.challenges) {
        if (challenge.name === name) {
          if (challenge.codingChallengeStatus < 1) {
            challenge.codingChallengeStatus = result.findIt
              ? 1
              : challenge.codingChallengeStatus;
          }
          if (challenge.codingChallengeStatus < 2) {
            challenge.codingChallengeStatus = result.fixIt
              ? 2
              : challenge.codingChallengeStatus;
          }
          this.calculateCodingProgressPercentage();
